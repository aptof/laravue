...
import { getVersion, getUser } from '@/services/api';
...
<template>
  <div
    ...
    <!-- <div class="fixed top-0 right-0 px-6 py-4 sm:block">
      ...
      <template v-else>
        <router-link :to="{ name: RouteNames.LOGIN }" class="text-sm text-gray-700 dark:text-gray-500 underline">Log in
        </router-link>
    ...
    <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="flex justify-center mt-4 sm:items-center sm:justify-between">
        ...
        <div class="ml-4 text-center text-sm text-gray-500 sm:text-right sm:ml-0">
          Laravel v{{ version?.laravel }} (PHP v{{ version?.php }})
        </div>
      </div>
    </div>
  </div>
